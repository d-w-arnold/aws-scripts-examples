...
usage() {
  echo "
    Git tag a new commit, and deploy (release) changes via the corresponding AWS CodePipeline pipeline.
    ...
    NB. Firstly, new git tag is applied to the HEAD commit of the specified git branch (in the specified git repo),
          ...
          for the corresponding AWS CodePipeline pipeline.
    ...
    Arguments:
      ...
      -g      The git repo name to checkout, in order to apply the new git tag (e.g. 'dog-gw').

      -t      The git tag, to add to the latest commit, of the git repo branch (e.g. suggested format: 'YYMMDD.<PROD_DEPLOY_NO>').

      -r      The AWS region in which the AWS CodePipeline pipeline resides (e.g. 'eu-west-2').

    Arguments:

    Example usages:

    ./aws-deploy.sh -r \"eu-west-2\" -g \"dog-gw\" -d \"prod\" -t \"220727.1\"
    ./aws-deploy.sh -r \"eu-west-2\" -g \"dog-gw-base\" -d \"prod\" -t \"220727.1\"
    ./aws-deploy.sh -r \"eu-west-2\" -g \"dog-gw\" -d \"uat-preview\" -t \"uat\"
    ./aws-deploy.sh -r \"eu-west-2\" -g \"dog-gw\" -d \"sih-demo\" -t \"sihd\"
  "
  exit 0
}
...
main() {
  ...
  printf "%s\n\n" "## Updating AWS Systems Manager Parameter Store with the new deployment git tag, and releasing changes on AWS CodePipeline pipeline"
  python3 ${PY_FILE} --repo "${GIT_REPO}" --deploy_env "${DEPLOY_ENV}" --region "${REGION}" --account "${AWS_ACCOUNT_ID}" --tag "${GIT_TAG}"
}
