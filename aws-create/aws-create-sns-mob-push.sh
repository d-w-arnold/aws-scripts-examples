...
usage() {
  echo "
    ...
    Arguments:
      ...
      -r      The AWS region in which to create the SNS platform application (e.g. 'eu-west-2').

    Arguments:

    Example usages:

    ./aws-create-sns-mob-push.sh -r \"eu-west-2\" -g \"dog-gw\" -d \"dev\"
    ./aws-create-sns-mob-push.sh -r \"eu-west-2\" -g \"dog-spain-gw\" -d \"staging\"
    ./aws-create-sns-mob-push.sh -r \"eu-west-2\" -g \"dog-gw\" -d \"sihp\"
    ./aws-create-sns-mob-push.sh -r \"eu-west-2\" -g \"cat-gw\" -d \"uat\"
  "
  exit 0
}
...
#
main() {
  FILENAME="aws-create-sns-mob-push"
  PY_FILE="${FILENAME}.py"
  ...
  if [[ -z "${GIT_REPO}" ]]; then
    ...
    while true; do
      echo ""
      read -r -p "Warning: You've specified a 'base' git repo name (for creating a SNS platform application). Are you sure you'd like to continue: '${GIT_TAG}' ? [y/n] " yn
      case $yn in
  ...
  printf "%s\n\n" "## Creating a new SNS platform application"
